...
??? example "PKIX Path Building Failed"
    ...
    - d. Using command prompt, navigate to this `Security` folder

    - e. Import the Trusted Root Certificate into your `cacerts` keystore, using following command :
    ...
    - f. Make sure in your INGenious `ingenious.bat` or/and `ingenious.command file`, the reference of the above `cacerts` is present.
...
??? example "Unable to Launch INGenious, Getting "Windows protected your PC" Pop-up When Clicking on ingenious.bat File in Windows"
    ...
    If you get the following pop-up when you click on ingenious.bat, right click on **ingenious.bat :material-arrow-right: Properties :material-arrow-right: Unblock :material-arrow-right: Apply**

    ![WindowsErrorPopUp](/img/faq/WindowsErrorPopUp.png "WindowsErrorPopUp"){ width="75%" } 
...
??? example "Alternative for @AfterTest (TestNG) in INGenious Playwright Studio" 
    ...
    ...
        public void afterReportComplete() throws Exception {
            // Enter your custom after-report code here     
        }
    ...
    }
    ```
